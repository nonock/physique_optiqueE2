...
L'énergie rayonnée est fournie par l'énergie mécanique de l'oscillateur qui
  ...
  relation avec celle du mouvement précédent.
...
Cette forme de distribution est attribuée aux collisions entre atomes de la
  source, collisions du à l'agitation thermique. Elle est reliée à la durée
...
$L_c$ est la longueur moyenne du train d'onde. Pour un système interférenciel
  ...
  rayons 1 et 2 issus des sources, est supérieure à $L_c$, alors il n'y a plus
...
Si la variation de $\delta$ pour les différentes sources ponctuelles est
  perturbée, cela peut brouiller les interférences comme nous les verrons avec
  les fentes d'Young.
...
Considérons deux ondes monochromatiques, polarisées rectilignement issus de deux
  sources $S_1$ et $S_2$ ponctuelles. Les ondes sont sphériques, synchrones
...
$$\vec{E_1} = \dfrac{C_1}{r_1} \exp{j(\omega t + kr_1 + \phi_1)} \vec{u_1}$$
$$\vec{E_2} = \dfrac{C_2}{r_2} \exp{j(\omega t + kr_2 + \phi_2)} \vec{u_2}$$
...
$$\vec{E_1} = A_1 \exp{(j \Phi_1)} \exp{(j \omega t)} \vec{u_1}$$
$$\vec{E_2} = A_2 \exp{(j \Phi_2)} \exp{(j \omega t)} \vec{u_2}$$
$$\vec{E} = (A_1 \exp{j \Phi_1} \vec{u_1} + A_2 \exp{j \Phi_2} \vec{u_2})
\exp{(j\omega t)}$$
$$I = n E E^*\text{ (conjugué) }$$
$$I = n \vec E \vec{E^*} = n (\vec{E_1} + \vec{E_2})(\vec{E^*_1} +
\vec{E^*_2}) \text{  on considère n = 1}$$
$$I = [A_1 \exp{(j \Phi_1)} \vec{u_1} + A_2 \exp{(j \Phi_2)} \vec{u_2}] .
[A_1 \exp{(-j \Phi_1)} \vec{u_1} + A_2 \exp{(-j \Phi_2)} \vec{u_2}]$$
$$I = A^2_1 + A^2_2 + A_1 A_2 \exp{(j(\Phi_1 - \Phi_2))} \vec{u_1} . \vec{u_2} +
A_2 A_1 \exp{(j(\Phi_2 - \Phi_1))} \vec{u_2} . \vec{u_1}$$
$$I_1 = E_1 E^*_1 = A^2_1$$
$$I_2 = E_2 E^*_2 = A^2_2$$
...
par $\gamma = \dfrac{I_{max} - I_{min}}{I_{max} + I_{min}}$
